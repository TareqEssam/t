...
class AssistantAI {
    ...
    showLicenseDetails(activityId) {
        console.log("🔍 استدعاء معلومات للفريق - المعرف:", activityId);
        ...
        if (typeof masterActivityDB !== 'undefined') {
            ...
            if (data && data.details) {
                ...
                // بناء نص معلوماتي بحت موجه للفريق
                const infoText = `
📌 **تقرير بيانات النشاط للفريق:**
-----------------------------------
🏢 **النشاط:** ${data.text}
🏛️ **جهة الاختصاص:** ${d.auth || 'غير محددة'}
📝 **المتطلبات:** ${d.req || 'لا يوجد قيود خاصة'}
⚖️ **السند القانوني:** ${d.leg || 'القوانين العامة'}
🔧 **طبيعة العمل:** ${d.act || 'نشاط فني/تجاري'}
-----------------------------------
⚠️ *هذا البيان للعرض فقط ولا يؤثر على طلبات الترخيص الحالية.*
                `;

                // استخدام sendMessage بدلاً من displayResponse لتجنب أخطاء الواجهة
                if (window.assistantUI && window.assistantUI.sendMessage) {
                    // نرسل الرد كرسالة من المساعد (Assistant Message)
                    window.assistantUI.receiveMessage(infoText); 
                } else {
                    // حل احتياطي إذا كانت الواجهة تستخدم مسمى آخر
                    console.log("البيانات المستخرجة:", infoText);
                    alert("بيانات النشاط: \n" + infoText);
                }
            }
        }
